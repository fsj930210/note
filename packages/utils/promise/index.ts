...
function resolvePromise(promise: MyPromise<any>, x: any, resolve: (v?: any) => void, reject: (r?: any) => void) {
  ...
  if (x instanceof MyPromise) { // eslint-disable-line
    if (x.status === PENDING) {
      // 如果是pending状态有可能被resolve掉, resolve的值得重新走一遍流程
      x.then(function (v) {
...
function defaultFulfilled(v?: any) {
  return v;
}
function defaultRejected(r: any) {
  throw r;
}
...
export default class MyPromise<T> {
  ...
  then(onFulfilled: (value?: any) => any = defaultFulfilled, onRejected: (reason: any) => any = defaultRejected) {
    ...
    if (this.status === FULFILLED) {
      // 只有在fulfilled状态下才调用onFulfilled方法
      promise2 = new MyPromise<any>((resolve, reject) => {
        setTimeout(() => {
          // 规范上定义then方法里面最佳实践是异步执行的, 刚好这里resolvePromise方法里面的promise2参数需要整个promise创建完毕才能拿到
          try {
  ...
  static all(promises: MyPromise<any>[]) {
    if (getType(promises) !== 'Array') {
      return new MyPromise((_, reject) => reject(new TypeError('You must pass an array to all.')));
    }
        ...
        MyPromise.resolve(p).then(
          (v) => {
            ...
            if (resolvedCount === len) {
              // 必须等所有promise都resolve了才能返回resolve
   ...
   * @memberof MyPromise
   */
  static retry(callback: (...args: any[]) => MyPromise<any> | PromiseLike<any>, count: number, delay = 2000) {
    return new MyPromise<any>((resolve, reject) => {
      function fn() {
        callback().then(
          (v: any) => {

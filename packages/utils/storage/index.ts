...
export default class MyStorage {
  ...
  public static set(arg: string | Record<string, any>, ...rest: any[]): void {
    ...
    if (typeof arg === 'string') {
      if (!rest.length) {
        throw new Error('你必须提供第二个参数来作为值使用');
      }
      if (rest.length === 2 && (typeof rest[1] !== 'number' || isNaN(rest[1]))) {
        throw new Error('第二个参数必须是一个数值来作为过期时间');
      }
      if (rest.length === 2 && rest[1] <= 0) {
        throw new Error('你设置了一个过期的时间');
      }
      if (rest.length > 2) {
        throw new Error('最多只能传入3个参数');
      }
      const value = rest[0];
